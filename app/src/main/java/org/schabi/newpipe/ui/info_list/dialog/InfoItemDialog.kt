package org.schabi.newpipe.ui.info_list.dialog
...
class InfoItemDialog private constructor(activity: Activity,
    ...
    init {
        ...
        if (info.uploaderName != null) {
            detailsView.text = info.uploaderName
        ...
        val items = entries.stream()
            .map<String> { entry: StreamDialogEntry -> entry.getString(activity) }
            .toArray<String> {size -> arrayOfNulls<String>(size) }
//            .toArray<String> { _Dummy_.__Array__() }
        ...
        val action = DialogInterface.OnClickListener { d: DialogInterface?, index: Int ->
            entries[index].action.onClick(fragment,
                info)
        }
    ...
    class Builder @JvmOverloads constructor(activity: Activity,
        ...
        fun addAllEntries(vararg newEntries: StreamDialogDefaultEntry): Builder {
            Stream.of(*newEntries).forEach { entry: StreamDialogDefaultEntry -> this.addEntry(entry) }
            return this
        }
        ...
        fun setAction(entry: StreamDialogDefaultEntry,
                      action: StreamDialogEntryAction
        ): Builder {
            for (i in entries.indices) {
                if (entries[i].resource == entry.resource) {
                    entries[i] = StreamDialogEntry(entry.resource, action)
                    return this
                }
            }
            return this
        ...
        fun addMarkAsWatchedEntryIfNeeded(): Builder {
            ...
            if (isWatchHistoryEnabled && !isLiveStream(infoItem.streamType)) {
        ...
        fun addPlayWithKodiEntryIfNeeded(): Builder {
            if (shouldShowPlayWithKodi(context, infoItem.serviceId)) {
        ...
        fun create(): InfoItemDialog {
            ...
            return InfoItemDialog(this.activity, this.fragment, this.infoItem, this.entries)
        }
        ...
        companion object {
            ...
            fun reportErrorDuringInitialization(throwable: Throwable?,
                ...
                showSnackbar(App.getApp().baseContext, ErrorInfo(
                    throwable!!,
    ...
    companion object {
        private val TAG: String = Build::class.java.simpleName
    }
}
