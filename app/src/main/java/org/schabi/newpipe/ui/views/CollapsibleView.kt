...
import org.schabi.newpipe.ui.ktx.animateHeight
import org.schabi.newpipe.util.Logd
import org.schabi.newpipe.ui.views.CollapsibleView.ViewMode.Companion.COLLAPSED
...
class CollapsibleView : LinearLayout {
    ...
    fun switchState() {
        if (!readyToChangeState) return
        if (currentState == COLLAPSED) expand()
        else collapse()
    }
    ...
    fun getDebugLogString(description: String?): String {
        return String.format("%-100s → %s", description,
