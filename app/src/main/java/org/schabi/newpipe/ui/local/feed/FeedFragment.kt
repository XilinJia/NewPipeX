...
package org.schabi.newpipe.ui.local.feed
...
import org.schabi.newpipe.ui.local.feed.service.FeedLoadService
...
class FeedFragment : BaseStateFragment<FeedState>() {
    ...
    private val listenerStreamItem = object : OnItemClickListener, OnItemLongClickListener {
        override fun onItemClick(item: Item<*>, view: View) {
            if (item is StreamItem && !isRefreshing) {
                ...
                NavigationHelper.openVideoDetailFragment(requireContext(), fm, stream.serviceId, stream.url, stream.title, null, false)
            }
        }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        onSettingsChangeListener = SharedPreferences.OnSharedPreferenceChangeListener { _, key ->
            if (getString(R.string.list_view_mode_key).equals(key)) updateListViewModeOnResume = true
        }
        PreferenceManager.getDefaultSharedPreferences(requireActivity()).registerOnSharedPreferenceChangeListener(onSettingsChangeListener)
    }
    ...
    override fun onViewCreated(rootView: View, savedInstanceState: Bundle?) {
        ...
        _binding = FragmentFeedBinding.bind(rootView)
        ...
        binding.itemsList.addOnScrollListener(
            object : RecyclerView.OnScrollListener() {
                override fun onScrollStateChanged(recyclerView: RecyclerView, newState: Int) {
                    // Check if we scrolled to the top
                    if (newState == RecyclerView.SCROLL_STATE_IDLE && !recyclerView.canScrollVertically(-1)) {
                        if (tryGetNewItemsLoadedButton()?.isVisible == true) hideNewItemsLoaded(true)
                    }
                }
            },
        )
    ...
    override fun onPause() {
        super.onPause()
        listState = binding.itemsList.layoutManager?.onSaveInstanceState()
    }
    ...
    override fun onResume() {
        ...
        if (updateListViewModeOnResume) {
            ...
            setupListViewMode()
            if (viewModel.stateLiveData.value != null) handleResult(viewModel.stateLiveData.value!!)
        }
    }
    ...
    override fun initListeners() {
        super.initListeners()
        binding.refreshRootView.setOnClickListener { reloadContent() }
        binding.swipeRefreshLayout.setOnRefreshListener { reloadContent() }
        binding.newItemsLoadedButton.setOnClickListener {
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        when (item.itemId) {
            ...
            R.id.menu_item_feed_toggle_played_items -> showStreamVisibilityDialog()
        }
    ...
    override fun onDestroy() {
        ...
        if (onSettingsChangeListener != null) {
            PreferenceManager.getDefaultSharedPreferences(requireActivity()).unregisterOnSharedPreferenceChangeListener(onSettingsChangeListener)
    ...
    override fun showLoading() {
        ...
        binding.loadingProgressText.animate(true, 200)
    ...
    override fun hideLoading() {
        ...
        binding.loadingProgressText.animate(false, 0)
    ...
    private fun handleProgressState(progressState: FeedState.ProgressState) {
        ...
        binding.loadingProgressText.text = when {
            !isIndeterminate -> "${progressState.currentProgress}/${progressState.maxProgress}"
            progressState.progressMessage > 0 -> getString(progressState.progressMessage)
            else -> "∞/∞"
        }

        binding.loadingProgressBar.isIndeterminate = isIndeterminate || (progressState.maxProgress > 0 && progressState.currentProgress == 0)
        binding.loadingProgressBar.progress = progressState.currentProgress
        binding.loadingProgressBar.max = progressState.maxProgress
    }
    ...
    private fun handleLoadedState(loadedState: FeedState.LoadedState) {
        ...
        listState?.run {
            binding.itemsList.layoutManager?.onRestoreInstanceState(listState)
            listState = null
        }

        val feedsNotLoaded = loadedState.notLoadedCount > 0
        binding.refreshSubtitleText.isVisible = feedsNotLoaded
        if (feedsNotLoaded) binding.refreshSubtitleText.text = getString(
        ...
        if (loadedState.items.isEmpty()) showEmptyState()
        else hideLoading()
    }
        ...
        errors.forEachIndexed { i, t ->
            if (t is FeedLoadService.RequestException && t.cause is ContentNotAvailableException) {
                disposables.add(Single.fromCallable {
                    ...
                    .subscribe({ subscriptionEntity -> handleFeedNotAvailable(subscriptionEntity, t.cause, errors.subList(i + 1, errors.size)) },
    ...
    private fun handleFeedNotAvailable(
        ...
        val sharedPreferences = PreferenceManager.getDefaultSharedPreferences(requireContext())
        val isFastFeedModeEnabled = sharedPreferences.getBoolean(getString(R.string.feed_use_dedicated_fetch_method_key), false)
        ...
        when (cause) {
            ...
            is ContentNotAvailableException -> {
                when {
                    ...
                    !isNullOrEmpty(cause.message) -> message += "\n" + cause.message
                }
            }
        }
    ...
    private fun updateRefreshViewState() {
        binding.refreshText.text = getString(
    ...
    private fun highlightNewItemsAfter(updateTime: OffsetDateTime) {
        ...
        if (highlightCount > 0) showNewItemsLoaded()
            ...
            ?.slideUp(250L, delay = 100,
                execOnEnd = {
                    ...
                    context?.let {
                        ...
                        if (DeviceUtils.hasAnimationsAnimatorDurationEnabled(it)) hideNewItemsLoaded(true, 10000)
                    }
                },
            )
    }
    ...
    private fun hideNewItemsLoaded(animate: Boolean, delay: Long = 0) {
        tryGetNewItemsLoadedButton()?.clearAnimation()
        if (animate) {
            tryGetNewItemsLoadedButton()?.animate(false, 200, delay = delay,
                execOnEnd = {
        ...
        } else tryGetNewItemsLoadedButton()?.isVisible = false
    }
    ...
    private fun tryGetNewItemsLoadedButton(): Button? {
        return _binding?.newItemsLoadedButton
    }
