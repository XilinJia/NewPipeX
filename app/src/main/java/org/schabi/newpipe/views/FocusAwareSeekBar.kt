...
import org.schabi.newpipe.util.DeviceUtils.isConfirmKey
...
class FocusAwareSeekBar : AppCompatSeekBar {
    ...
    override fun setOnSeekBarChangeListener(l: OnSeekBarChangeListener?) {
        this.listener = if (l == null) null else NestedListener(l)

        super.setOnSeekBarChangeListener(listener)
    }

    override fun onKeyDown(keyCode: Int, event: KeyEvent): Boolean {
        if (!isInTouchMode && isConfirmKey(keyCode)) {
            releaseTrack()
        }
    ...
    private val touchModeListener = OnTouchModeChangeListener { isInTouchMode: Boolean ->
        if (isInTouchMode) {
            releaseTrack()
        }
    }
