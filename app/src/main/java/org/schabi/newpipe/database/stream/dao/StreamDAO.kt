...
abstract class StreamDAO : BasicDAO<StreamEntity> {
    ...
    abstract fun getStream(
        serviceId: Long,
        url: String,
    ): Flowable<List<StreamEntity>>

    @Query("UPDATE streams SET uploader_url = :uploaderUrl WHERE url = :url AND service_id = :serviceId")
    abstract fun setUploaderUrl(
        serviceId: Long,
        url: String,
        uploaderUrl: String,
    ): Completable
    ...
    internal abstract fun getMinimalStreamForCompare(
        serviceId: Int,
        url: String,
    ): StreamCompareFeed?
    ...
    private fun compareAndUpdateStream(newerStream: StreamEntity) {
        val existentMinimalStream =
            getMinimalStreamForCompare(newerStream.serviceId, newerStream.url)
