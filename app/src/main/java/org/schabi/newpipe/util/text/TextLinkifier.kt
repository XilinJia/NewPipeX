...
object TextLinkifier {
    ...
    @JvmField
    val SET_LINK_MOVEMENT_METHOD: Consumer<TextView> =
        Consumer { v: TextView -> v.movementMethod = LongPressLinkMovementMethod.instance }
    ...
    fun fromPlainText(textView: TextView,
        ...
        changeLinkIntents(textView, textView.text, relatedInfoService,
            relatedStreamUrl, disposables, onCompletion)
    }
    ...
    private fun changeLinkIntents(textView: TextView,
        ...
        disposables.add(Single.fromCallable {
            ...
            val textBlockLinked =
                SpannableStringBuilder(chars)
            val urls = textBlockLinked.getSpans(0, chars.length,
                URLSpan::class.java)

            for (span in urls) {
                val url = span.url
                val longPressClickableSpan: LongPressClickableSpan =
                    UrlLongPressClickableSpan(context, disposables, url)
            ...
            .subscribe(
                { textBlockLinked: SpannableStringBuilder? ->
                    setTextViewCharSequence(textView,
                        textBlockLinked,
                        onCompletion)
                },
                { throwable: Throwable? ->
    ...
    private fun addClickListenersOnTimestamps(
        ...
        while (timestampsMatches.find()) {
            val timestampMatchDTO =
                TimestampExtractor.getTimestampFromMatcher(timestampsMatches, descriptionText) ?: continue
