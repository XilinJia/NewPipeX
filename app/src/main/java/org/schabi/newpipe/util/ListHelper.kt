...
object ListHelper {
    ...
    fun getDefaultAudioTrackGroup(context: Context,
        ...
        val highestRanked = groupedAudioStreams.stream()
            .max { o1: List<AudioStream?>?, o2: List<AudioStream?>? ->
                cmp.compare(
                    o1!![0], o2!![0])
            }
            .orElse(null)
    ...
    fun <S : Stream?> getPlayableStreams(
            streamList: List<S>?, serviceId: Int
    ): List<S> {
        ...
        return getFilteredStreamList(streamList
            ...
            (stream!!.deliveryMethod != DeliveryMethod.TORRENT && (stream.deliveryMethod != DeliveryMethod.HLS
                    ...
                    && (serviceId != youtubeServiceId || stream.itagItem == null || SUPPORTED_ITAG_IDS.contains(
                stream.itagItem!!.id)))
        }
    }
    ...
    fun getGroupedAudioStreams(
        ...
        return collectedStreams.values.stream()
            .sorted { o1: List<AudioStream>, o2: List<AudioStream> ->
                nameCmp.compare(o1[0], o2[0])
            }
    ...
    private fun computeDefaultResolution(context: Context, key: Int,
        ...
        var resolution = if (preferences != null
        ) preferences.getString(context.getString(key), context.getString(value))
        else context.getString(R.string.best_resolution_key)

        val maxResolution = getResolutionLimit(context)
        if (maxResolution != null
                && (resolution == context.getString(R.string.best_resolution_key) || compareVideoStreamResolution(
                    maxResolution,
                    resolution!!) < 1)) {
    ...
    fun getSortedStreamVideosList(
        ...
        val videoStreamsOrdered =
            ...
            else Arrays.asList(videoOnlyStreams, videoStreams)
    ...
    private fun sortStreamList(videoStreams: List<VideoStream>,
        ...
        val comparator = Comparator.nullsLast(Comparator
            .comparing({ obj: VideoStream -> obj.getResolution() }, { obj: String?, r1: String? ->
                compareVideoStreamResolution(obj?:"", r1?:"")
            })
    ...
    fun getAudioFormatComparator(
            defaultFormat: MediaFormat?, limitDataUsage: Boolean
    ): Comparator<AudioStream?> {
        val formatRanking = if (limitDataUsage
        ) AUDIO_FORMAT_EFFICIENCY_RANKING else AUDIO_FORMAT_QUALITY_RANKING

        var bitrateComparator =
            Comparator.comparingInt { obj: AudioStream? -> obj!!.averageBitrate }
        if (limitDataUsage) {
            bitrateComparator = bitrateComparator.reversed()
        }

        return Comparator.comparing(
            { obj: AudioStream? -> obj!!.format }, { o1: MediaFormat?, o2: MediaFormat? ->
                if (defaultFormat != null) {
                    return@comparing java.lang.Boolean.compare(o1 == defaultFormat, o2 == defaultFormat)
                }
                0
            }).thenComparing(bitrateComparator).thenComparingInt { stream: AudioStream? ->
    ...
    fun getAudioTrackComparator(
        ...
        ) AUDIO_TRACK_TYPE_RANKING_DESCRIPTIVE else AUDIO_TRACK_TYPE_RANKING

        return Comparator.comparing({ obj: AudioStream? -> obj!!.audioTrackType },
            { o1: AudioTrackType?, o2: AudioTrackType? ->
            ...
            Comparator.nullsFirst(Comparator.comparing { locale: Locale? -> locale!!.isO3Language == langCode }))
            ...
            .thenComparing({ obj: AudioStream? -> obj!!.audioLocale },
                Comparator.nullsFirst(Comparator.comparing { locale: Locale? -> locale!!.isO3Language == Locale.ENGLISH.isO3Language }))
    }
