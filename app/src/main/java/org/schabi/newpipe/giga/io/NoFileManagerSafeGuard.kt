...
object NoFileManagerSafeGuard {
    ...
    private fun showActivityNotFoundAlert(context: Context?) {
        requireNotNull(context) { "Unable to open no file manager alert dialog: Context is null" }
        // Android 10+ only allows SAF
        val message = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) context.getString(R.string.no_appropriate_file_manager_message_android_10)
