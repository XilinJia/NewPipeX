package org.schabi.newpipe.giga.streams
...
class Mp4FromDashWriter(vararg sources: SharpStream) {
    ...
    init {
        for (src in sources) {
            if (!src.canRewind() && !src.canRead()) throw IOException("All sources must be readable and allow rewind")
        }
    ...
    fun selectTracks(vararg trackIndex: Int) {
        if (isDone) throw IOException("already done")
        if (tracks != null) throw IOException("tracks already selected")
    ...
    fun build(output: SharpStream) {
        ...
        val singleSampleBuffer = if (tracks!!.size == 1 && tracks!![0]!!.kind == TrackKind.Audio) tracks!![0]!!.trak!!.mdia!!.mdhdTimeScale / 1000 else -1
        ...
        for (i in readers!!.indices) {
            ...
            while ((readers!![i]?.getNextChunk(true).also { chunk = it }) != null) {
                ...
                while ((chunk.nextSampleInfo.also { info = it }) != null) {
                    ...
                    if (info.sampleDuration > defaultSampleDuration[i]) defaultSampleDuration[i] = info.sampleDuration
            ...
            if (defaultMediaTime[i] < 1) defaultMediaTime[i] = defaultSampleDuration[i]

            readers!![i]!!.rewind()

            if (singleSampleBuffer > 0) initChunkTables(tablesInfo[i], singleSampleBuffer, singleSampleBuffer)
            else initChunkTables(tablesInfo[i], SAMPLES_PER_CHUNK_INIT.toInt(), SAMPLES_PER_CHUNK.toInt())
            ...
            } else tablesInfo[i]!!.stszDefault = 0

            if (tablesInfo[i]!!.stss == tablesInfo[i]!!.stsz) tablesInfo[i]!!.stss = -1 // for audio tracks (all samples are keyframes)
            // ensure track duration
            if (tracks!![i]!!.trak!!.tkhd!!.duration < 1) tracks!![i]!!.trak!!.tkhd!!.duration = sampleExtra[i].toLong() // this never should happen
        }
        ...
        if (auxSize < THRESHOLD_MOOV_LENGTH) auxBuffer = ByteBuffer.allocate(auxSize) // cache moov in the memory
        ...
        if (auxBuffer == null) outSeek(ftypSize.toLong())
        ...
        for (i in readers!!.indices) {
            ...
            if (tablesInfo[i]!!.sbgp > 0) writeEntryArray(tablesInfo[i]!!.sbgp, 1, sampleCount[i])
        }

        if (auxBuffer == null) outRestore()
        ...
        while (written > 0) {
            ...
            for (i in readers!!.indices) {
                ...
                var syncCount = 0
                val limit = if (singleSampleBuffer > 0) singleSampleBuffer
                else (if (sampleIndex[i] == 0) SAMPLES_PER_CHUNK_INIT else SAMPLES_PER_CHUNK).toInt()
                ...
                while (j < limit) {
                    ...
                    if (tablesInfo[i]!!.ctts > 0) {
                        if (sample.info!!.sampleCompositionTimeOffset == sampleExtra[i]) sampleCount[i]++
                        else {
                            if (sampleExtra[i] >= 0) {
                    ...
                    if (tablesInfo[i]!!.stss > 0 && sample.info!!.isKeyframe) sync[syncCount++] = sampleIndex[i]
                    if (tablesInfo[i]!!.stsz > 0) sizes[j] = sample.data.size
                ...
                if (j > 0) {
                    written++
                    if (tablesInfo[i]!!.stsz > 0) tablesInfo[i]!!.stsz = writeEntryArray(tablesInfo[i]!!.stsz, j, *sizes)
                    if (syncCount > 0) tablesInfo[i]!!.stss = writeEntryArray(tablesInfo[i]!!.stss, syncCount, *sync)

                    if (tablesInfo[i]!!.stco > 0) {
                        if (is64) tablesInfo[i]!!.stco = writeEntry64(tablesInfo[i]!!.stco, chunkOffset)
                        else tablesInfo[i]!!.stco = writeEntryArray(tablesInfo[i]!!.stco, 1, chunkOffset.toInt())
                    }
                    outRestore()
                }
            }
    ...
    private fun outBackup() {
        if (auxBuffer == null && lastWriteOffset < 0) lastWriteOffset = writeOffset
    }
    ...
    private fun initChunkTables(tables: TablesInfo?, firstCount: Int, successiveCount: Int) {
        ...
        tables.stsc = 1
        if (firstCount != successiveCount) tables.stsc++
        if (remain) tables.stsc++
    ...
    private fun outSeek(offset: Long) {
        when {
            ...
            else -> throw IOException("cannot seek or rewind the output stream")
        }
    }
    ...
    private fun auxWrite(value: Int) {
        auxWrite(ByteBuffer.allocate(4).putInt(value).array())
    }

    @Throws(IOException::class)
    private fun auxWrite(buffer: ByteArray) {
        when {
            moovSimulation -> writeOffset += buffer.size.toLong()
            auxBuffer == null -> outWrite(buffer, buffer.size)
            else -> auxBuffer!!.put(buffer)
        }
    }
    ...
    private fun auxSeek(offset: Int) {
        when {
            moovSimulation -> writeOffset = offset.toLong()
            auxBuffer == null -> outSeek(offset.toLong())
            else -> auxBuffer!!.position(offset)
        }
    }
    ...
    private fun auxSkip(amount: Int) {
        when {
            moovSimulation -> writeOffset += amount.toLong()
            auxBuffer == null -> outSkip(amount.toLong())
            else -> auxBuffer!!.position(auxBuffer!!.position() + amount)
        }
    }
    ...
    private fun makeMoov(defaultMediaTime: IntArray, tablesInfo: Array<TablesInfo?>, is64: Boolean): Int {
        ...
        for (i in durations.indices) {
            ...
            if (durations[i] > longestTrack) longestTrack = durations[i]
        }

        makeMvhd(longestTrack)

        for (i in tracks!!.indices) {
            if (tracks!![i]!!.trak!!.tkhd!!.matrix.size != 36) throw RuntimeException("bad track matrix length (expected 36) in track n°$i")
    ...
    @Throws(IOException::class)
    private fun makeMdia(mdia: Mdia?, tablesInfo: TablesInfo?, is64: Boolean, isAudio: Boolean) {
        ...
        if (moovSimulation) {
            ...
            if (tablesInfo!!.stss > 0) make(0x73747373, -1, 1, tablesInfo.stss)
            if (tablesInfo.ctts > 0) make(0x63747473, -1, 2, tablesInfo.ctts)
            ...
            if (tablesInfo.stss > 0) tablesInfo.stss = make(0x73747373, -1, 1, tablesInfo.stss)
            if (tablesInfo.ctts > 0) tablesInfo.ctts = make(0x63747473, -1, 2, tablesInfo.ctts)
