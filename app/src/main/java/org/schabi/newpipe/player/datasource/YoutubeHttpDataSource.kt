...
class YoutubeHttpDataSource private constructor(private val connectTimeoutMillis: Int,
    ...
    class Factory : HttpDataSource.Factory {
        ...
        override fun setDefaultRequestProperties(
                defaultRequestPropertiesMap: Map<String, String>
        ): Factory {
        ...
        fun setAllowCrossProtocolRedirects(
                allowCrossProtocolRedirectsValue: Boolean
        ): Factory {
        ...
        fun setContentTypePredicate(
                contentTypePredicateToSet: Predicate<String>?
        ): Factory {
        ...
        fun setTransferListener(
                transferListenerToUse: TransferListener?
        ): Factory {
    ...
    override fun open(dataSpecParameter: DataSpec): Long {
        ...
        if (responseCode < 200 || responseCode > 299) {
            ...
            if (responseCode == 416) {
                ...
                if (dataSpecParameter.position == documentSize) {
                    ...
                    return if (dataSpecParameter.length != C.LENGTH_UNSET.toLong()
        ...
        val bytesToSkip = if (!rangeParameterEnabled) {
            ...
            if (responseCode == 200 && dataSpecParameter.position != 0L
        ...
        if (!isCompressed) {
            if (dataSpecParameter.length != C.LENGTH_UNSET.toLong()) {
                ...
                val contentLength = HttpUtil.getContentLength(
                    ...
                    httpURLConnection.getHeaderField(HttpHeaders.CONTENT_RANGE))
                bytesToRead = if (contentLength != C.LENGTH_UNSET.toLong()
    ...
    private fun makeConnection(dataSpecToUse: DataSpec): HttpURLConnection {
        ...
        while (redirectCount++ <= MAX_REDIRECTS) {
            val httpURLConnection = makeConnection(url, httpMethod, httpBody,
    ...
    private fun makeConnection(
        ...
        httpURLConnection.requestMethod = if (isAndroidStreamingUrl || isIosStreamingUrl
        ) "POST"
        else DataSpec.getStringForHttpMethod(httpMethod)
    ...
    private class NullFilteringHeadersMap(private val headers: Map<String, List<String>>) : ForwardingMap<String, List<String>>() {
        ...
        override val keys: MutableSet<String>
            get() = Sets.filter<String>(super.keys, Predicate { obj: String? -> obj != null })

        override val entries: MutableSet<MutableMap.MutableEntry<String, List<String>>>
            get() = Sets.filter<MutableMap.MutableEntry<String, List<String>>>(super.entries) { entry: MutableMap.MutableEntry<String, List<String>> -> entry.key != null }
        ...
        private fun maybeTerminateInputStream(connection: HttpURLConnection?,
            ...
            try {
                ...
                if (("com.android.okhttp.internal.http.HttpTransport\$ChunkedInputStream"
                    ...
                    val unexpectedEndOfInput = Assertions.checkNotNull(superclass).getDeclaredMethod(
