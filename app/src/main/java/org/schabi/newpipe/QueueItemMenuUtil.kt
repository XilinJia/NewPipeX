...
object QueueItemMenuUtil {
    ...
    fun openPopupMenu(playQueue: PlayQueue,
        ...
        val themeWrapper =
            ContextThemeWrapper(context, R.style.DarkPopupMenu)
        ...
        popupMenu.setOnMenuItemClickListener { menuItem: MenuItem ->
            when (menuItem.itemId) {
                ...
                R.id.menu_item_details -> {
                    ...
                    openVideoDetail(context!!, item.serviceId,
                ...
                R.id.menu_item_channel_details -> {
                    fetchUploaderUrlIfSparse(context!!, item.serviceId,
                        item.url, item.uploaderUrl
                    )  // An intent must be used here.
                    // Opening with FragmentManager transactions is not working,
                    // as PlayQueueActivity doesn't use fragments.
                    { uploaderUrl: String? ->
                        openChannelFragmentUsingIntent(
                            context, item.serviceId, uploaderUrl, item.uploader
                        )
                    }
                    return@setOnMenuItemClickListener true
                }
