...
class StatisticsPlaylistFragment
    : BaseLocalListFragment<List<StreamStatisticsEntry?>?, Void?>(), PlaylistControlViewHolder {
    ...
    private fun processResult(results: List<StreamStatisticsEntry>): List<StreamStatisticsEntry>? {
        val comparator = when (sortMode) {
            StatisticSortMode.LAST_PLAYED -> Comparator.comparing(
                StreamStatisticsEntry::latestAccessDate)
    ...
    override fun onResume() {
        super.onResume()
        if (activity != null) {
            setTitle(requireActivity().getString(R.string.title_activity_history))
        }
    }
    ...
    override val listHeader: ViewBinding?
        get() {
            headerBinding = StatisticPlaylistControlBinding.inflate(requireActivity().layoutInflater,
            itemsList, false)
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        if (item.itemId == R.id.action_history_clear) {
        ...
        if (infoItem is StreamStatisticsEntry) {
            val onDelete = recordManager!!
                ...
                .subscribe(
                    {
                        if (view != null) {
                            Snackbar.make(requireView(), R.string.one_item_deleted,
